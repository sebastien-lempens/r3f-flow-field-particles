...
const GpgpuFragmentShader = /*glsl*/ `
  ...
  void main() {
      ...
      if(particle.a >= 1.0) {
        ...
        vec3 flowField = vec3(
          snoise(vec4(particle.xyz + 0.0, uTime * 0.2)),
          snoise(vec4(particle.yzx + 0.0, uTime * 0.2)),
          snoise(vec4(particle.zxy + 0.0, uTime * 0.2))
        );
        flowField = normalize(flowField);
        //particle.xyz += flowField * uDeltaTime * 0.1;
...
const ParticlesFragmentShader = /*glsl*/ `
  ...
  void main() {
      ...
      if (vColor[0].x > 0.0 && vColor[1].x > 0.0) {
        color = mix(vColor[0], vColor[1], vViewPosition.y);
      } else {
        color = vModelColor;
      }
...
const FlowFieldParticles = ({ model, colors }) => {
  if (!model) throw "<FlowFieldParticles model={missing} /> ";
  ...
  const gpgpu = useMemo(() => {
    ...
    for (let i = 0; i < modelGeometry.count; i++) {
      ...
      dataTexture.image.data[i * 4 + 3] = 0;
    }
  ...
  useEffect(() => {
    if (particlesMaterialRef.current) {
      const colorsArray = colors.map(color => new Color(color)) || null;
  ...
  return (
    <>
      ...
      {/* <Points ref={particleslRef} range={modelGeometry.count} positions={gpgpu.texture}>
        <bufferGeometry>
          <bufferAttribute attach='attributes-position' count={modelGeometry.count} array={particleslRef.current?.geometry.attributes.position.array} itemSize={3} />
          <bufferAttribute attach='attributes-aParticlesUv' count={modelGeometry.count} array={particles.uvAttribute} itemSize={2} />
        </bufferGeometry>
        <particlesMaterial ref={particlesMaterialRef} />
      </Points> */}
      <Plane visible={false} args={[1, 1, 1, 1]} position={[0, 0, 0]} rotation={[0, 0, 0]}>
