...
import { BufferGeometry, BufferAttribute, Color, Uniform, Vector2 } from "three";
...
const GpgpuFragmentShader = /*glsl*/ `
  ...
  void main() {
    ...
    if (particle.a >= 1.0) {
        ...
        float uRepelStrength = 0.35;

        // Génération du flowField pour l'animation
        vec3 flowField = vec3(
            snoise(vec4(particle.xyz, timer) * 3.0),
            snoise(vec4(particle.yxz, timer) * 3.0),
            snoise(vec4(particle.zxy, timer) * 3.0)
        );
        ...
        vec2 direction = particlePos - mousePos;
        float distance = length(direction);
        vec2 repelDirection = normalize(direction) * 1.0;
        float repelForce = uRepelStrength / distance;  // Atténuation de la répulsion avec la distance
        particle.xy += repelDirection * repelForce * uDeltaTime;
...
const FlowFieldParticles = ({ colors, position = [0, 0, 0], size = 0.1, chaosIntensity = 0.3, scale = 1, children }) => {
  ...
  useFrame(({ clock, pointer }, delta) => {
    mouseAttenutation.current.lerp(pointer, 0.4);
    //mouseAttenutation.current.multiplyScalar(0.1);
    //console.log(mouseAttenutation.current, pointer.clone());

    const elapsedTime = clock.getElapsedTime();
    if (particlesMaterialRef.current) {
      gpgpu.particlesVariable.material.uniforms.uTime.value = elapsedTime;
      gpgpu.particlesVariable.material.uniforms.uDeltaTime.value = delta;
      gpgpu.particlesVariable.material.uniforms.uMouse.value = mouseAttenutation.current;
  ...
  return (
    <group position={position} scale={scale}>
      ...
      <Plane visible={false} args={[3, 3, 1, 1]} position={[2, 0, 0]} rotation={[0, 0, 0]}>
        <meshBasicMaterial map={gpgpu.texture} />
      </Plane>
      {/* <mesh geometry={childrenMeshRef.current.geometry} onPointerMove={onMove}></mesh> */}
    </group>
  );
};
